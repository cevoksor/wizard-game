...
export class Player {
  ...
  canFire() {
    return this.fireCooldown <= 0 && !this.isHit && this.animState === "idle";
  }
  fire() {
  ...
  update(dt, hooks) {
    ...
    const speed = this.isSprinting ? BASE_SPEED * SPRINT_MULTIPLIER : BASE_SPEED;
  ...
  draw(canvas, ctx) {
    ...
    if (this.shieldCharges > 0 && this.animState === "idle") {
      ctx.save();
